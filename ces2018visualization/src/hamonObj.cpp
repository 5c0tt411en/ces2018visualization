...
hamonObj::hamonObj(string _type, float _drawP, float _stay, float _erase) {
    type = _type;
    drawP = _drawP;
    stay = _stay;
    erase = _erase;
...
void hamonObj::draw() {
    ...
    ofRotateX(step * 360 * rotateRan.x);
    ofRotateY(step * 360 * rotateRan.y);
    ofRotateZ(step * 360 * rotateRan.z);
...
void hamonObj::resetTimer() {
    timeStamp = ofGetElapsedTimef();
}

float hamonObj::easeOutCubic(float t, float b, float c, float d) {
    t /= d ;
    t-- ;
    return (c*(t*t*t + 1) + b) ;
}
